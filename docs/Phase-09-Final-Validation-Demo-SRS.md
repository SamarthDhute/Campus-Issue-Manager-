# Smart Campus Issue Manager --- Phase 9 SRS: Final Validation & Demo

## 1. Purpose

This phase delivers **Final Validation & Demo** as a connected part of
the Smart Campus Issue Manager. This document defines both product
...
### FR-01 End-to-end lifecycle`\n`{=tex}`\nThe `{=tex}system shall provide end-to-end lifecycle according to the product workflow, business rules and role permissions.

### FR-02 All 11 automation validations`\n`{=tex}`\nThe `{=tex}system shall provide all 11 automation validations according to the product workflow, business rules and role permissions.

### FR-03 Role journeys`\n`{=tex}`\nThe `{=tex}system shall provide role journeys according to the product workflow, business rules and role permissions.

### FR-04 AI-human collaboration`\n`{=tex}`\nThe `{=tex}system shall provide ai-human collaboration according to the product workflow, business rules and role permissions.

### FR-05 SLA/risk/escalation`\n`{=tex}`\nThe `{=tex}system shall provide sla/risk/escalation according to the product workflow, business rules and role permissions.

### FR-06 Communication/resolution`\n`{=tex}`\nThe `{=tex}system shall provide communication/resolution according to the product workflow, business rules and role permissions.

### FR-07 Dashboards/insights`\n`{=tex}`\nThe `{=tex}system shall provide dashboards/insights according to the product workflow, business rules and role permissions.

### FR-08 Integration testing`\n`{=tex}`\nThe `{=tex}system shall provide integration testing according to the product workflow, business rules and role permissions.

### FR-09 Security testing`\n`{=tex}`\nThe `{=tex}system shall provide security testing according to the product workflow, business rules and role permissions.

### FR-10 Demo environment`\n`{=tex}`\nThe `{=tex}system shall provide demo environment according to the product workflow, business rules and role permissions.
...
-   `demo_seed_metadata(id UUID PK, scenario_name, version, created_at)`
...
``` text
V9__phase_09_final_validation_demo.sql
```
...
## 8. API Contract
  ...
  ----------------------- --------------------------------- -----------------------
  `GET`                   `/api/v1/system/health`           Final health check

  `GET`                   `/api/v1/system/readiness`        Final readiness check

  `GET`                   `/api/v1/demo/scenarios/{name}`   Verify demo scenario

  `POST`                  `/api/v1/demo/reset`              Reset demo data in
                                                            non-production only
  ---------------------------------------------------------------------------------
...
Requester
  ↓
Water leakage issue + photos
  ↓
AI analysis / summary / missing info / related cases
  ↓
Operator reviews recommendations
  ↓
Smart assignment
  ↓
Requester supplies missing information
  ↓
Inspection task + investigation
  ↓
SLA risk detected
  ↓
Team Lead intervention / escalation
  ↓
Resolution + evidence
  ↓
Requester confirms
...
CLOSED
  ↓
Manager reviews dashboard + insight
```

The final validation must prove that the features form one connected
workflow.

## 10. Detailed Implementation

-   Seed deterministic demo data for the water-leakage scenario and all
    roles.
-   Automate end-to-end API tests across role permissions.
-   Run frontend integration tests against a test backend.
-   Provide non-production-only controls for simulating SLA/risk
    conditions.
-   Verify every required automation produces a visible, traceable
    outcome.
-   Run security, resilience, migration, regression and deployment smoke
    tests.
...
-   Water leakage end-to-end journey
-   All 11 automations
-   Role matrix
-   AI unavailable
-   SLA/escalation
...
Phase 9 is complete only when the phase works end-to-end through Flutter
